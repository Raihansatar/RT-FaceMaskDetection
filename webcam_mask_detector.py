# Python file for detect the video camera as input and predict
...
# load the face mask detector model from disk maskNet = load_model("mask_detector_model.model")
...
while True:
	# grab the frame from the threaded video stream and resize it to have a maximum width of 1024 pixels
	frame = vs.read()
	...
	for (box, pred) in zip(locs, preds):
		...
		# determine the class label and color we'll use to draw the bounding box and text label = "Mask" if mask > incorrect_mask else "Incorrect Mask"
...
# do a bit of cleanup, destroy all windows
